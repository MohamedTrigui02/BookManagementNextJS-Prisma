...
<h1 className="text-3xl font-semibold">Editeurs</h1>
<Link
...
<li key={editeur.id} className="py-2 flex justify-between w-full">
<span>
<strong>{editeur.maisonedit}</strong>
</span>
<span className="flex gap-2">
<Link className="text-blue-700 underline hover:no-underline"

href={`/${editeur.id}/edit`}>Edit</Link>

<Link className="text-red-500 underline hover:no-underline"

href={`/${editeur.id}/delete`}>Delete</Link>

</span>
</li>
))}
{editeurs?.length < 1 && <div className="py-2">No data</div>}
</ul>
</div>
)
}
